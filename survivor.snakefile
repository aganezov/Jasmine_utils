...
rule survivor_retain_specific:
    output: os.path.join(output_dir, "{exp_name}.survivor.postp.specific.vcf")
...
rule survivor_postp:
    output: os.path.join(output_dir, "{exp_name}.survivor.postp.vcf")
...
rule survivor:
    output: os.path.join(output_dir, "{exp_name}.survivor.vcf")
    input: os.path.join(output_dir, "{exp_name}.survivor.input_vcf.txt")
    log: os.path.join(output_dir, "log", "{exp_name}.survivor.vcf.log")
    benchmark: repeat(os.path.join(output_dir, "benchmark", "{exp_name}.survivor.txt"), 5)
    params:
